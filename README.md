...
62. - 프로듀서: 메세지 발행자
    - 컨슈머: 메세지 소비자
    - 브로커: 메세지 저장, 전달
    - 주키퍼: 컨슈머와 통신, 카프카 메타데이터 저장, 카프카 상태관리 등 처리
    - 토픽: 데이터 구분을 위해 사용하는 단위
    - 파티션: 토픽 내부의 데이터 저장 공간
    - 데이터: 레코드
63. 카프카 + 레디스 조합을 사용하기 위해 docker-compose 파일을 만든다
64. 생성한 docker-compose 컨테이너를 구동하여 정상 구동되는지 테스트
65. 스프링 클라우드에서 메세지를 발행하고 소비하는 네 개의 컴포넌트 (스프링 버전이 올라감에 따라 deprecated)
    - 소스: 메세지 발행
    - 채널: 메세지 생산자가 발행하거나 메세지 소비자가 소비한 후 메세지를 보관할 큐를 추상화한것 즉 채널은 메세지를 보내고 받는 큐
    - 바인더: 특정 메세지 플랫폼과 통신하는 스프링 코드 바인더 부분은 메세지를 발행하고 소비한다
    - 싱크: 큐에서 메세지를 받는다 싱크는 들어오는 메세지 채널을 수신 대기 한다
66. 메세지 생산자 작성 (User, Board, Reply) 
    - 카프카 관련 클래스, 설정은 kafka 주석으로 표시 (스프링 버전이 올라감에 따라 deprecated되어 책의 예제로 생성하지 않음)
67. StreamBridge클래스를 이용하여 producer를 생성했다
    - 인터넷 예제의 설정 yml은 토이 프로젝트의 producer에서 작동하지 않는듯 하다 (나중에 고급 세팅이 필요할때 알아봐야함)
    - docker-compose.yml에서 생성한 topic은 작동하지 않는다
68. 컨슈머 코드 작성
69. 분산 캐싱을 구축하기 위해 레디스를 사용한다 (클라우드 캐싱 + 메시징)
    - 장점
     - 데이터 검색 성능 향상
     - 데이터를 보유한 데이터베이스 테이블에 대한 부하를 줄인다
     - 데이터베이스에 성능 문제가 있다면 정상적으로 저하될 수 있도록 회복력을 높인다
70. 레디스 의존성 추가
     - spring-data-redis
71. 컨피그 서버에 레디스 설정을 기입한다
72. 각 서비스에 레디스 설정 클래스 작성
73. 트랜잭션 추적을 위해 스프링 클라우드 슬루스 의존성 추가
    - 코드로 작성했던 상관관계를 슬루스가 대신해준다 (로깅, 모든 아웃바운드 HTTP 호출, 스프링 메시징 채널의 메시지)
