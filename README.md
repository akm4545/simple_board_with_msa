...
73. 트랜잭션 추적을 위해 스프링 클라우드 슬루스 의존성 추가
    ...
    - spring-cloud-starter-sleuth (스프링 3버전은 micrometer를 사용해야 함)
    - 슬루스 의존성 추가 시 추적 분석 예시
    - [애플리케이션 이름, 추적ID=상관관계 ID에 해당하는 용어/한 트랜잭션에서 고유한 번호, 스팬 ID = 전체 트랜잭션의 일부를 나타내는 고유 ID / 트랜잭션에 속한 각 서비스에는 고유한 스팬
         ID가 부여되며 스팬 ID는 특히 집킨과 통합할 때 트랜잭션을 시각화 하는데 관련이 많다, 내보내기 = 집킨에 추적 데이터 전송 여부를 나타내는 지표]
...
76. docker-compose에 ELK 스택 추가
    - E = 일레스틱서치 (모든 유형의 데이터 분산 분석 엔진)
    - L = 로그스태시 (데이터 추가 및 수집하고 엘라스틱 서치에 인덱싱되기 전에 로그를 변환할 수 있는 서버 사이드 데이터 프로세싱 파이프라인)
    - K = 키바나 (일레스틱서치용 시각화 및 데이터 관리도 도구)
77. 키바나 구성을 위해 http://localhost:5601/로 접속
    - (남은 디스크 용량이 5%이하면 키바나가 읽기전용 모드로 실행되어 로그스태시와 연동 불가)
    - (로그를 한번 발생시켜서 데이터를 키바나에 보내줘야함)
    - 왼쪽 탭에서 discover 아이콘 클릭
    - index pattern 클릭
    - 
